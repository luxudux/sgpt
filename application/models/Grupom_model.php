...
class Grupom_model extends CI_Model{
   function __construct() {
     parent::__construct();
     $this->table='gruposm_cat';
   }
   //Mostrar lista
   public function mostrar_lista(){
     $table=$this->table;
     $this->db->select('id,nombre,bloqueado,
     (SELECT COUNT(id_grupom) FROM materia_prima_cat WHERE materia_prima_cat.id_grupom=gruposm_cat.id) AS uso');
