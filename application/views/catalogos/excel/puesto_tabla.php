...
$spreadsheet->getProperties()->setCreator('SGPT')
    ...
    ->setCategory('Reporte');
// Rename worksheet
$spreadsheet->getActiveSheet()->setTitle('Puestos');
...
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , 'LISTA DE PUESTOS');
$fila++;
$spreadsheet->setActiveSheetIndex(0);
$activeSheet = $spreadsheet->getActiveSheet()
...
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , $puesto[$i]->id)
              ->setCellValue('B'.$fila , $puesto[$i]->nombre);
              $fila++;
  }
