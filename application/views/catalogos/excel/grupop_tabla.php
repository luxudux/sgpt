...
$filename="Lista-de-grupos de productos";
//Colores de fondo
...
$spreadsheet->getProperties()->setCreator('SGPT')
    ...
    ->setCategory('Reporte');
// Rename worksheet
$spreadsheet->getActiveSheet()->setTitle('Grupos');
...
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , 'LISTA DE GRUPOS DE PRODUCTOS');
$fila++;
$spreadsheet->setActiveSheetIndex(0);
$activeSheet = $spreadsheet->getActiveSheet()
...
for($i=0; $i<count($grupop); $i++) {
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , $grupop[$i]->id)
              ->setCellValue('B'.$fila , $grupop[$i]->nombre);
              $fila++;
  }
