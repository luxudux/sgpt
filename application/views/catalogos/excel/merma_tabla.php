...
$spreadsheet->getProperties()->setCreator('SGPT')
    ...
    ->setCategory('Reporte');
// Rename worksheet
$spreadsheet->getActiveSheet()->setTitle('Mermas');
...
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , 'LISTA DE MERMAS');
$fila++;
$spreadsheet->setActiveSheetIndex(0);
$activeSheet = $spreadsheet->getActiveSheet()
...
for($i=0; $i<count($merma); $i++) {
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , $merma[$i]->id)
              ->setCellValue('B'.$fila , $merma[$i]->nombre);
              $fila++;
  }
