...
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , 'LISTA DE PROVEEDORES');
$fila++;
$spreadsheet->setActiveSheetIndex(0);
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , 'Id')
              ->setCellValue('B'.$fila , 'Nombre')
              ->setCellValue('C'.$fila , 'Correo')
...
$activeSheet = $spreadsheet->getActiveSheet()
              ->setCellValue('A'.$fila , $proveedor[$i]->id)
              ->setCellValue('B'.$fila , $proveedor[$i]->nombre)
              ->setCellValue('C'.$fila , $proveedor[$i]->correo)
              ->setCellValue('D'.$fila , $proveedor[$i]->telefono);
              $fila++;
  }
...
$spreadsheet->getActiveSheet()->getStyle('C'.$inF.':'.'D'.$fila)
       ->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
